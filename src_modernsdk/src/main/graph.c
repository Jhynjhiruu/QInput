...
putchar(u16 color, int curs_x, int curs_y, char c)
{
  ...
  u8	*pc = pat[c-0x20];
  u16	*p = (u16 *)osViGetNextFramebuffer() + x + 320*(y-8);
...
{
  ((u16 *)osViGetNextFramebuffer())[x + 320*y]=color;
}
